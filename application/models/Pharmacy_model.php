...
//---------------------------Server side code datatable--------------------------------------
    ...
    public function search_datatable() {
        ...
        $this->db->group_by('medicine_category.id');
        $this->db->order_by('medicine_category.id');
    ...
    public function totalQuantity($pharmacy_id) {
        $query = $this->db->select('sum(available_quantity) as total_qty, sum(stock) as main_stock, status as status_p')
                ->where('pharmacy_id', $pharmacy_id)
                ->get('medicine_batch_details');
        return $query->row_array();
    }

    public function getBatchIdQuantity($medicine_id) {
        $query = $this->db->select('available_quantity, stock, temp_qty')
