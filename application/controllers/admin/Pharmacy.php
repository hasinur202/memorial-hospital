...
class Pharmacy extends Admin_Controller {
    ...
    public function search() {
        ...
        foreach ($resultlist as $value) {
            ...
            $totalAvailableStock = $available_qty['main_stock'];
            $resultlist[$i]["main_stock"] = $totalAvailableStock;

            $status = $available_qty['status'];
...
public function dt_search(){
    ...
    if ($this->rbac->hasPrivilege('staff', 'can_add')) {
        if($result_value->status_p == 'pending'){
            $action.="<a href='#' onclick='viewDetail(".$result_value->id.")' class='btn btn-default btn-xs' data-toggle='tooltip' title='".$this->lang->line('show')."' ><i class='fa fa-check'></i></a>"; 
        }
    }
    ...
    // }
        ...
        $nestedData[]=$result_value->status_p;
        $nestedData[]=$result_value->main_stock;
        $data[] = $nestedData;
}
...
//======================================================================================
    ...
    public function getMedicineBatchPending() {
        if (!$this->rbac->hasPrivilege('add_medicine_stock', 'can_view')) {
            access_denied();
        }
        $id = $this->input->post("pharmacy_id");
        ...
        $this->load->view('admin/pharmacy/pendingMedicine', $data);
    }
    ...
    public function addBill() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $net_purchase = $this->input->post('net_purchase');
            $total_purchase = $net_amount - $net_purchase;
    ...
    public function billReport() {
        ...
        if (empty($search_type) || empty($patient_type)) {
            ...
            $resultlist = $this->report_model->searchSpecificReport($select, $join = array(), $table_name, $search_type, $patient_type, $search_table, $search_column, $patient_column, $where = array());
        }
    ...
    public function updateSupplierBill() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($id)) {
                ...
                $quantity = $this->input->post('quantity');
                // $total_quantity = $this->input->post('available_quantity');
                $total_quantity = $this->input->post('stock');
                $amount = $this->input->post('amount');
                $purchase_price = $this->input->post('purchase_price');
                $data_array1 = array();
...
public function stocktransfer(){
        ...
        $this->form_validation->set_rules('transfer_qty', $this->lang->line('qty'), 'required');
        
        if ($this->form_validation->run() == FALSE) {
            $msg = array(
                'medicine_id' => form_error('medicine_id'),
                'transfer_qty' => form_error('transfer_qty'),
            );
            $array = array('status' => 'fail', 'error' => $msg, 'message' => '');
        } else {
            $phar_id = $this->input->post('medicine_id');
            $phar_qty = $this->input->post('transfer_qty');

            $available_qty = $this->pharmacy_model->getBatchIdQuantity($phar_id);

            if($phar_qty > $available_qty['stock']){
                $array = array('status' => 'fail', 'error' => '', 'message' => '');
            }else{
                $update_main_stock = $available_qty['stock']-$phar_qty;
                $update_phar_qty = $available_qty['temp_qty']+$phar_qty;

                $update_data = array('id' => $this->input->post('medicine_id'), 'status' => 'pending', 'temp_qty' => $update_phar_qty, 'stock' => $update_main_stock);
